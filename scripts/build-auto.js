...
function buildForRuntime(runtime, version) {
  ...
  try {
    ...
    console.error(
      `✗ Failed to build for ${runtime} ${version}:`,
      error.message
    );
    return false;
  }
...
function main() {
  ...
  if (buildForRuntime(runtimeInfo.runtime, runtimeInfo.version)) {
    ...
    // Try different Electron versions as fallback
    const fallbackVersions = ["38.1.0", "37.4.0", "37.3.1"];

    for (const version of fallbackVersions) {
      console.log(`Trying Electron ${version} fallback...`);
      if (buildForRuntime("electron", version)) {
